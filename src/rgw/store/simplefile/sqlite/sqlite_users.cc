...
std::string SQLiteUsers::getDBPath() const {
  auto rgw_simplefile_path = ceph_context_->_conf.get_val<std::string>("rgw_simplefile_data_path");
  auto db_path = std::filesystem::path(rgw_simplefile_path) / std::string(USERS_DB_NAME);
