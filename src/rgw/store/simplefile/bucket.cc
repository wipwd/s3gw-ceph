...
int SimpleFileBucket::check_quota(const DoutPrefixProvider *dpp,
  ...
  ldpp_dout(dpp, 10) << __func__ << ": user(max size: " << user_quota.max_size
                     << ", max objs: " << user_quota.max_objects
                     << "), bucket(max size: " << bucket_quota.max_size
                     << ", max objs: " << bucket_quota.max_objects
