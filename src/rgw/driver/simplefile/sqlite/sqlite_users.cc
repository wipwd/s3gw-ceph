...
SQLiteUsers::SQLiteUsers(CephContext* cct) : ceph_context_(cct) {
  sync();
}

std::optional<DBOPUserInfo> SQLiteUsers::getUser(const std::string& userid
) const {
...
std::optional<DBOPUserInfo> SQLiteUsers::getUserByEmail(const std::string& email
) const {
...
std::optional<DBOPUserInfo> SQLiteUsers::getUserByAccessKey(
...
std::string SQLiteUsers::getDBPath() const {
  auto rgw_simplefile_path =
      ceph_context_->_conf.get_val<std::string>("rgw_simplefile_data_path");
  auto db_path =
      std::filesystem::path(rgw_simplefile_path) / std::string(USERS_DB_NAME);
