            ...
            if (is_histogram(data.type)) {
              ...
              if (data.type & PERFCOUNTER_TIME) {
                return fmt::format(
                    ...
                    "{}</li><li>avg: {}</li></ul>",
                    std::chrono::nanoseconds(sum), count,
                    std::chrono::nanoseconds(static_cast<uint64_t>(avg)));
              } else {
                return fmt::format(
                    "<ul><li>sum: {}</li><li>count: "
                    "{}</li><li>avg: {:.2f}</li></ul>",
              ...
              if (data.type & PERFCOUNTER_U64) {
                ...
                return fmt::format("{:d}.{:09d}s", data.u64 / 1000000000ull,
          ...
          auto format_scalar = [&]() {
            ...
            if (is_longrunavg(data.type)) {
              ...
              if (data.type & PERFCOUNTER_U64) {
                ...
                return fmt::format("{:d}.{:09d}", data.u64 / 1000000000ull,
                                   data.u64 % 1000000000ull);
              } else {
...
# TYPE {name} {type}
          ...
          if (perf_counters == perfcounter_ops_svc_time_hist) {
            ...
            fmt::print(os, "{name}{labels} {value}\n", "name"_a = name,
